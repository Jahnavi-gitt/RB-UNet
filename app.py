...
Supports both local execution (HTTPServer) and Vercel serverless deployment (WSGI / BaseHTTPRequestHandler).
"""
...
# ---------------------------------------------------------------------------
# Global Model Caching (keeps warmed PyTorch weights across requests)
# ---------------------------------------------------------------------------
...
def get_baseline_model(device):
    """Loads and caches the baseline standard U-Net model."""
    ...
    if _baseline_model_cache is None:
        ...
        if ckpt_path.exists():
            ...
            _baseline_model_cache = build_model_from_checkpoint(str(ckpt_path), device=str(device))
        else:
            features = [16, 32, 64, 128, 256]
            _baseline_model_cache = UNet(in_channels=3, out_channels=1, features=features).to(device)
        _baseline_model_cache.eval()
    return _baseline_model_cache


def get_rb_model(device):
    """Loads and caches the proposed RB-UNet model."""
    global _rb_model_cache
    if _rb_model_cache is None:
        ...
        if ckpt_path.exists():
            ...
            _rb_model_cache = build_model_from_checkpoint(str(ckpt_path), device=str(device))
        else:
            features = [16, 32, 64, 128, 256]
            _rb_model_cache = RBUNet(in_channels=3, out_channels=1, features=features, boundary_head_channels=16).to(device)
        _rb_model_cache.eval()
    return _rb_model_cache


# ---------------------------------------------------------------------------
# Core Pure-Payload Functions (Shared across Local Server and Vercel)
# ---------------------------------------------------------------------------
...
def segment_image_payload(body: dict) -> tuple:
    """Runs live segmentation inference on an uploaded image with strict original alignment."""
    try:
        image_b64 = body.get("image")
        if not image_b64:
            return {"success": False, "error": "Missing image base64"}, 400
        ...
        import traceback
        traceback.print_exc()
        return {"success": False, "error": "Segmentation could not be generated. Check the uploaded image and try again."}, 500


def live_robustness_payload(body: dict) -> tuple:
    ...
    try:
        ...
        base_model = get_baseline_model(device)
        t0 = time.time()
        with torch.no_grad():
        ...
        import traceback
        traceback.print_exc()
        return {"success": False, "error": "Live robustness evaluation could not be processed for this image."}, 500


def resolve_request_path(raw_path: str, headers) -> str:
    """Resolves the intended API route path from direct or rewritten URL/headers."""
    parsed = urlparse(raw_path)
    req_path = parsed.path

    # If routed to index or api root via rewrite, check query param or Vercel routing headers
    if req_path in ("/api", "/api/", "/api/index.py", "/"):
        qs = parse_qs(parsed.query)
        if "path" in qs:
            target = qs["path"][0]
            req_path = target if target.startswith("/") else "/" + target
        else:
            matched = headers.get("x-matched-path") or headers.get("x-forwarded-uri") or headers.get("x-now-route-matches")
            if matched:
                req_path = urlparse(matched).path
    return req_path


# ---------------------------------------------------------------------------
# HTTP Server Handler (Local Development & BaseHTTPRequestHandler runtime)
# ---------------------------------------------------------------------------

class RBUNetServerHandler(SimpleHTTPRequestHandler):
    """Custom HTTP handler serving UI and REST API."""

    # Maintain staticmethod bindings for backward-compatibility
    get_baseline_model = staticmethod(get_baseline_model)
    get_rb_model = staticmethod(get_rb_model)
    ...
    def do_GET(self):
        req_path = resolve_request_path(self.path, self.headers)

        if req_path == "/api/results":
            self.handle_get_results()
        elif req_path == "/api/status":
            self.handle_get_status()
        elif req_path.startswith("/outputs/"):
            file_rel = req_path.replace("/outputs/", "")
    ...
    def do_POST(self):
        req_path = resolve_request_path(self.path, self.headers)
        if req_path == "/api/segment":
            self.handle_segment_image()
        elif req_path == "/api/robustness":
            self.handle_live_robustness()
        else:
            self.send_error(404, f"Unknown API endpoint: {req_path}")

    def handle_get_status(self):
        self.send_json_response(get_status_payload())

    def handle_get_results(self):
        self.send_json_response(get_results_payload())
    ...
    def handle_segment_image(self):
        try:
            ...
            body = {}
        res, status = segment_image_payload(body)
    ...
    def handle_live_robustness(self):
        try:
            ...
            body = {}
        res, status = live_robustness_payload(body)
        self.send_json_response(res, status=status)
...
def wsgi_app(environ, start_response):
    ...
    # Route resolution from query param or Vercel rewrite headers
    if path in ("/api", "/api/", "/api/index.py", "/"):
        ...
        if "path" in qs:
            ...
            if matched:
                path = urlparse(matched).path

    # Clean query string if contained in path
    if "?" in path:
        path = path.split("?", 1)[0]
    ...
    if method == "GET":
        ...
        if clean_path == "/api/status":
            ...
            if file_path.exists() and file_path.is_file():
                ...
                file_headers = [("Content-Type", mime), ("Cache-Control", "public, max-age=3600"), *cors_headers]
                start_response("200 OK", file_headers)
                with open(file_path, "rb") as f:
                    return [f.read()]
            else:
                start_response("404 Not Found", json_headers)
                return [json.dumps({"error": f"File not found: {file_rel}"}).encode("utf-8")]
            ...
            if index_path.exists():
                ...
                with open(index_path, "rb") as f:
                    return [f.read()]
            start_response("404 Not Found", json_headers)
            return [json.dumps({"error": "Index file not found"}).encode("utf-8")]
        elif path.startswith("/css/") or path.startswith("/js/"):
            static_file = UI_DIR / path.lstrip("/")
            if static_file.exists() and static_file.is_file():
                mime = "text/css" if path.startswith("/css/") else "application/javascript"
                start_response("200 OK", [("Content-Type", mime), *cors_headers])
                with open(static_file, "rb") as f:
                    return [f.read()]
            start_response("404 Not Found", json_headers)
            return [json.dumps({"error": f"Static file not found: {path}"}).encode("utf-8")]
        else:
            start_response("404 Not Found", json_headers)
        ...
        if clean_path == "/api/segment":
            res, status_code = segment_image_payload(body)
            status_str = "200 OK" if status_code == 200 else f"{status_code} Error"
            start_response(status_str, json_headers)
            return [json.dumps(res).encode("utf-8")]
        elif clean_path == "/api/robustness":
            res, status_code = live_robustness_payload(body)
            status_str = "200 OK" if status_code == 200 else f"{status_code} Error"
            start_response(status_str, json_headers)
            return [json.dumps(res).encode("utf-8")]
        else:
            start_response("404 Not Found", json_headers)
            return [json.dumps({"error": f"Unknown API endpoint: {path}"}).encode("utf-8")]

    else:
        start_response("405 Method Not Allowed", json_headers)
...
# ---------------------------------------------------------------------------
# CLI Diagnostics & Local Server Startup
# ---------------------------------------------------------------------------
...
def serve():
    """Runs local development HTTPServer."""
